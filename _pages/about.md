...
Hi! I'm **Lei Liu** (刘磊), a senior at **Arcadia University** pursuing a double major in **Mathematics** and **Computer Science**. I build **LLM agents** for **clinical and biomedical** applications — high-stakes settings where being *capable* is not enough: to be trusted, a system must also know its limits and stay safe. I pursue this trustworthiness from three angles: **uncertainty quantification**, so a model knows when it might be wrong and can defer; **causal inference**, so it draws valid cause-and-effect conclusions from messy clinical data rather than mere correlations; and **security**, so agents stay reliable and within their authorized bounds. My goal is to build the next generation of agents and large language models that can be genuinely deployed in clinical treatment and biomedical research.
